"""
Multi-seed experiment: address Reviewer 2.1.

Run the main 1D comparison (supervised, supervised_10pct, sort_match_ssl)
with 3 seeds each, report mean +/- std.
...
def main():
    ...
    for variant in ["supervised", "supervised_10pct", "sort_match_ssl"]:
        ...
        results[variant] = {"maes": maes, "mean": np.mean(maes), "std": np.std(maes)}

    print(f"\n{'='*60}")
    print(f"  MULTI-SEED SUMMARY (3 seeds: {SEEDS})")
    print(f"{'='*60}")
    ...
    with open(os.path.join(RESULTS_DIR, "multiseed_results.json"), "w") as f:
        json.dump(results, f, indent=2)
    print(f"\nResults saved to {RESULTS_DIR}/multiseed_results.json")
